# SteamShell 1.5.0 Release Test Checklist
...
- Confirm missing current options and `[SteamShell] SettingsSchemaVersion=6` are added.
...
- With `EnableMouseParkOnBoot=true`, the pointer is parked at the right edge of the active display once during startup.
...
- Move the pointer over a visible Steam tile, launch a game, and close it normally. Confirm Steam may restore itself
  without a forced activation, the pointer moves to the right edge within about one second, and the old tile is no
