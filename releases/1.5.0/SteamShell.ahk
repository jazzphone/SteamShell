...
global SteamShellVersion := "1.5.0"
...
global CurrentSettingsSchemaVersion := 6
...
[SteamShell]
SettingsSchemaVersion=6                                    ; Internal schema used for safe settings upgrades
...
ParkMouseRightEdge(targetHwnd := 0) {
 ...
 GetMouseParkMonitorBounds(targetHwnd, &left, &top, &right, &bottom)
 ; Preserve the configured logical offset, but clamp it ourselves to the last
 ; pixel of the target monitor. This cannot spill onto a display to its right.
 desiredX := right + MouseParkRightOffsetPx
 x := Min(Max(desiredX, left), right - 1)
 y := Min(
...
QuickMenuGetDefinitions() {
 ...
 if (QuickMenuPage = "SETTINGS_RTSS") {
     ...
     rows.Push(Map("id", "qFrameCap", "label", "RTSS Frame Cap Label"))
...
PollController() {
 ...
 if (settingsControllerActive) {
     if (SettingsEditorDialogActive || !(buttons & 0x0020)) {
         ...
         downTick["LT"] := 0, longFired["LT"] := false, prevTrigDown["LT"] := false
         downTick["RT"] := 0, longFired["RT"] := false, prevTrigDown["RT"] := false
         SettingsEditorHandleController(pressed, lx, ly, rx, ry, settingsCategoryDirection)
         return
     }
 }
...
; ==============================================================================
MonitorShell() {
 global AllowExplorer, SteamLaunched
...
SettingsEditorAddMappedChoice(category, section, key, label, choices, values, &y, defaultValue := "") {
 ...
 ctrl := SettingsGui.AddDropDownList("x485 y" y " w260", displayChoices)
 ctrl.Choose(selectedIndex)
 ctrl.OnEvent("Change", SettingsEditorMarkDirty)
 SettingsEditorRegisterControl(category, labelCtrl)
 SettingsEditorRegisterControl(category, ctrl)
...
SettingsEditorHandleController(pressed, lx := 0, ly := 0, rx := 0, ry := 0, settingsCategoryDirection := 0) {
 ...
 ; mouse here. LT/RT are reserved for categories, so RT is not a speed boost.
 if (rx != 0 || ry != 0) {
     speed := ControllerMouseSpeed
     dx := Round((rx / 32767.0) * speed)
...
GetSteamShellHealthResults() {
 ...
 HealthResult(results, A_IsAdmin ? "pass" : "warn", "Runtime privileges",
     A_IsAdmin
         ...
         : "SteamShell is not elevated; Windows may block control of elevated applications.")
...
ShowSettingsEditor(*) {
 ...
 SettingsEditorAddHeading(category, "RTSS & Performance"
     , "SteamShell sends RTSS shortcuts; the frame-cap value below is a display label and does not configure RTSS.")
 y := 150
