...
This is **1.9.9**, locked with standalone SteamShell in this self-contained
release bundle. Controller input, the Quick Menu, Settings,
...
1.9.9 retains settings schema 8 and consolidates the validated 0.1.21 feature
...
`Validate-SharedParity.ps1` at the repository root.

HDR behaviour is **unchanged**: the audit briefly added the Win+Alt+B fallback
that standalone has, which was a misreading — see *Display & HDR* below. The
companion still reports **Unavailable** rather than sending a blind toggle, and
`Validate-SteamShell-XFE.ps1` asserts that the chord appears nowhere in this
tree.
...
hardening described above; the current 1.9.9 working tree uses schema 8. Use
the locked release directory when reproducibility matters.
...
`dist\SteamShell-XFE.exe`, then verifies the executable reports file version
1.9.9.0. The build automatically embeds
`assets\SteamShell-XFE.ico`; `-IconPath` can still override it for a custom
build.

From this release bundle's root, `Run-SteamShellValidation.cmd` validates and
builds both SteamShell platforms,
exercises broken-source and stale-output rejection in both trees, verifies both
file versions, and publishes both verified EXEs together to that root's
`current\` only after the complete run passes. A failed run returns non-zero and
preserves the previous `current\` directory.
