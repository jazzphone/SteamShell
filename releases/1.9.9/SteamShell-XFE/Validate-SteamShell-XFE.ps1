...
$buildLauncherPath = Join-Path $projectRoot "Build-SteamShell-XFE.cmd"
$buildScriptPath = Join-Path $projectRoot "Build-SteamShell-XFE.ps1"
...
Assert-True (Test-Path $buildLauncherPath) "The XFE double-clickable build launcher is missing."
Assert-True (Test-Path $buildScriptPath) "Build-SteamShell-XFE.ps1 is missing."
...
Assert-True (
    ...
    $buildScript -match 'SteamShell-XFE version verification failed.*?1\.9\.9\.0') (
...
Assert-True (
    ...
    $sample -match '(?m)^PresetFrameCap=158$' -and
    $source -match
...
Assert-True (
    ...
    $source -match
        '(?s)mappingActive\s*:=\s*viewDown\s*\|\|\s*EnablePersistentMouseMode.*?' +
        'if\s*!mappingActive') (
...
Assert-True (
    ...
    $source -match
        ...
        'ResetControllerHoldState\(downTick,\s*longFired,\s*triggerDown') (
...
Assert-True ($sample -match '(?m)^ControllerDeadzone=3000$') (
...
$standaloneSource = Join-Path (Split-Path -Parent $projectRoot) "SteamShell\SteamShell.ahk"
if (Test-Path $standaloneSource) {
    Assert-True (Test-Path $sharedParityScript) (
        "The sibling SteamShell tree exists, but Validate-SharedParity.ps1 is missing.")
    & $sharedParityScript -Quiet:$Quiet
} elseif (-not $Quiet) {
    Write-Host "Shared parity: skipped (the SteamShell tree is not beside this one)."
}
