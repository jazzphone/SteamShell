...
Assert-True (
    ...
    $helperSource -match '@Ahk2Exe-SetVersion 1\.9\.9\.1' -and
    $helperSource -match 'HelperVersion\s*:=\s*"1\.9\.9"' -and
    $source -match 'ElevatedHelperExpectedVersion\s*:=\s*"1\.9\.9\.1"') (
...
$helperFunctionMatches = [regex]::Matches(
    $helperSource,
    '(?m)^([A-Za-z_][A-Za-z0-9_]*)\([^\r\n{}]*\)\s*\{')
$helperDuplicates = $helperFunctionMatches |
    Group-Object { $_.Groups[1].Value.ToLowerInvariant() } |
    Where-Object Count -gt 1
Assert-True ($helperDuplicates.Count -eq 0) (
    "Duplicate helper functions: " +
    (($helperDuplicates | ForEach-Object Name) -join ", "))
...
Assert-True (
    $source -match
        '(?s)ShowSetupAssistant\([^)]*\).*?\+Resize.*?' +
        'SetupAssistantInitializeScrolling.*?GetSafeTargetWindowDpi.*?' +
        'workHeightPhysical\s*\*\s*0\.80.*?measuredOuterHeight.*?' +
        'safeOuterHeight.*?SetupAssistantGui\.Show\("Hide w760 h"' -and
    $source -match
        '(?s)GetSafeTargetWindowDpi\([^)]*\).*?GetDpiForWindow.*?' +
        'MonitorFromWindow.*?GetDpiForMonitor' -and
    $source -match
        '(?s)SetupAssistantInitializeScrolling\(\).*?ClassScrollBar.*?' +
        'SetupAssistantVerticalScroll.*?SetupAssistantMouseWheel' -and
    $source -match 'vSetupSteamPath' -and
    $source -match 'vSetupRtssPath' -and
    $source -match
        '(?s)SetupAssistantProgramFilesX86\(\).*?ProgramFiles\(x86\)' -and
    $source -match
        '(?s)SetupAssistantDiscoverSteamPath\(\).*?' +
        'SetupAssistantProgramFilesX86.*?' +
        '\\Steam\\steam\.exe.*?Valve\\Steam' -and
    $source -match
        '(?s)SetupAssistantDiscoverRtssPath\(\).*?' +
        'SetupAssistantProgramFilesX86.*?' +
        'RivaTuner Statistics Server\\RTSS\.exe') (
    "Setup Assistant scrolling, visible application paths, or default discovery is missing.")
Assert-True (
    $source -match
        '(?s)SetupAssistantRefreshDeployment\([^)]*\).*?' +
...
Assert-True (
    $source -match
        '(?s)CleanupTemporaryUpgradeSidecar\([^)]*\).*?' +
        'GetAbsoluteSteamShellPath.*?' +
        ...
        'SetupState.*?pending.*?inprogress.*?' +
        'DirDelete\(sourceDataDirectory,\s*true\)' -and
    $source -match
        '(?s)SteamShellPathUsesLinkOrJunction\([^)]*\).*?' +
        'FileGetAttrib\(candidate\).*?"L"' -and
    $source -match
        '(?s)GetAbsoluteSteamShellPath\([^)]*\).*?' +
        'GetFullPathNameW.*?GetLongPathNameW' -and
    $source -match
        '(?s)DeploySteamShell\([^)]*\).*?' +
        ...
        'RequestSteamShellRestart.*?' +
        'IntentionalExitMode\s*:=\s*"upgrade-complete".*?ExitApp' -and
    $source -match
        '(?s)ShowSetupCompletionDialog\([^)]*\).*?' +
        'Restart Now.*?Restart Later' -and
    $source -match
        '(?s)RequestSteamShellRestart\([^)]*\).*?' +
        'System32\\shutdown\.exe.*?/r /t 0') (
    "Upgrade cleanup, intentional updater exit, or Restart Now/Later completion flow is incomplete.")
Assert-True (
    $source -match
        '(?s)SetupAssistantMsgBox\([^)]*\).*?Owner.*?SettingsEditorDialogActive' -and
    $source -match
        '(?s)DeploySteamShell\([^)]*\).*?SetupAssistantMsgBox' -and
    $source -match
        '(?s)SetupAssistantLaunchExternal\([^)]*\).*?-AlwaysOnTop.*?' +
        'WinMinimize.*?LaunchInteractiveApp.*?SetupAssistantRestoreAfterExternal' -and
    $source -match
        '(?s)SetupAssistantOpenUacSettings\([^)]*\).*?' +
        'SetupAssistantLaunchExternal') (
    "Setup confirmations or external/UAC foreground handoff is disconnected.")
Assert-True (
    $source -match
        '(?s)StoreWindowsAutoLogonSecret\([^)]*\).*?LsaOpenPolicy.*?' +
        'DefaultPassword.*?LsaStorePrivateData.*?RtlSecureZeroMemory' -and
    $source -match
        '(?s)ValidateWindowsLogonCredentials\([^)]*\).*?LogonUserW' -and
    $source -match
        '(?s)ConfigureWindowsAutoLogon\([^)]*\).*?' +
        'ValidateWindowsLogonCredentials.*?DefaultUserName.*?' +
        'DefaultDomainName.*?StoreWindowsAutoLogonSecret.*?' +
        'RegWrite\("1",\s*"REG_SZ".*?AutoAdminLogon' -and
    $source -match
        '(?s)catch\s+as\s+err.*?RegWrite\("0",\s*"REG_SZ".*?' +
        'StoreWindowsAutoLogonSecret\("",\s*true,\s*&rollbackError\)' -and
    $source -match
        '(?s)DisableWindowsAutoLogon\([^)]*\).*?AutoAdminLogon.*?' +
        'RegDelete\(winlogonKey,\s*"DefaultPassword"\).*?' +
        'StoreWindowsAutoLogonSecret\("",\s*true' -and
    $source -match
        '(?s)SetupAssistantConfigureAutoLogon\([^)]*\).*?Password\s+' +
        'vAutoLogonPassword.*?AutoLogonDialogEnable' -and
    $source -notmatch
        'RegWrite\([^\r\n]*"DefaultPassword"') (
    "The protected, transactional Auto-Login implementation is incomplete or writes a plaintext password.")
Assert-True (
    $source -match
        '(?s)ReadElevatedHelperPreference\(\)\s*\{.*?' +
        'EnableElevatedInputHelper.*?"true".*?return ToBool' -and
    $source -match
        '(?s)StartElevatedInputHelper\(\)\s*\{.*?' +
        'ReadElevatedHelperPreference\(\).*?' +
        'if\s*!EnableElevatedInputHelper.*?return false.*?' +
        'ExtractEmbeddedElevatedHelper.*?\*RunAs' -and
    $source -match
        '(?s)SettingsEditorAddCheckbox\(\s*category,\s*"Features",\s*' +
...
Assert-True (
    $source -match
        '(?s)GetRetiredIniKeys\(\).*?RunElevatedOnStartup.*?' +
...
Assert-True (
    ...
    $source -match
        '(?s)ExtractEmbeddedElevatedHelper\([^)]*\).*?' +
        'FileGetVersion.*?ElevatedHelperExpectedVersion.*?FileMove' -and
    $source -match
        '(?s)DeploySteamShell\([^)]*\).*?' +
        'SteamShell-Helper\.exe.*?ExtractEmbeddedElevatedHelper') (
    "The compiled helper is no longer embedded, version-verified, and deployed by Setup.")
Assert-True (
    $source -match
        '(?s)RegisterElevatedHelperTask\([^)]*\).*?' +
        'InteractiveToken.*?HighestAvailable.*?AllowStartOnDemand.*?' +
        'schtasks\.exe.*?/create' -and
    $source -match
        '(?s)StartElevatedHelperTask\([^)]*\).*?' +
        'schtasks\.exe.*?/run.*?WaitForNewExecutablePid' -and
    $source -match
        '(?s)StartElevatedInputHelper\(\).*?mode\s*=\s*"standard".*?' +
        'StartElevatedHelperTask' -and
    $source -match
        '(?s)VerifyElevatedHelperProcess\([^)]*\).*?ProcessGetPath.*?' +
        'GetProcessTokenSecurity.*?helperIntegrity\s*!=\s*"High".*?' +
        'ExpectedInteractiveUserSid.*?ExpectedInteractiveSessionId' -and
    $source -match
        '(?s)StartElevatedInputHelper\(\).*?' +
        'WaitForVerifiedElevatedHelper.*?ElevatedHelperAvailable' -and
    $source -match
        '(?s)StartElevatedInputHelper\(\).*?\*RunAs') (
...
Assert-True (
    ...
    $source -match
        'HealthResult\(results,\s*helperStatus,\s*"Elevated helper"') (
    "Health Check no longer reports the separated main/helper privilege state.")
Assert-True (
    $source -match
        '(?s)InitializeExpectedInteractiveIdentity\(\)' -and
    $source -match
        '(?s)FirstRunSetupMode\s*\{.*?if\s*!A_IsAdmin\s*\{.*?' +
        'PromptForAdministratorSetupAndExit\(\).*?' +
        'CloseExistingSteamShellInstancesForElevatedSetup' -and
    $source -match
        '(?s)SetupAssistantApply\([^)]*\).*?if\s*!A_IsAdmin.*?' +
        'PromptForAdministratorSetupAndExit\(\)' -and
    $source -match
        '(?s)DeploySteamShell\([^)]*\).*?if\s*!A_IsAdmin.*?' +
        'PromptForAdministratorSetupAndExit\(\)' -and
    $source -match
        '(?s)PromptForAdministratorSetupAndExit\([^)]*\).*?' +
        'WriteAdministratorSetupRequestMarker.*?' +
        'Please Start SteamShell As Administrator for First Install or Upgrade.*?' +
        'IntentionalExitMode\s*:=\s*"setup-admin-required".*?ExitApp' -and
    $source -match
        '(?s)ConsumeAdministratorSetupRequestMarker\([^)]*\).*?' +
        'UserSid=.*?SessionId=.*?ExpectedInteractiveUserSid.*?' +
        'ExpectedInteractiveSessionId' -and
    $source -match
        '(?s)ElevatedSetupMatchesInteractiveDesktop\([^)]*\).*?' +
        'explorer\.exe.*?desktopIntegrity.*?medium.*?' +
        'ExpectedInteractiveUserSid' -and
    $source -match
        '(?s)CloseExistingSteamShellInstancesForElevatedSetup\([^)]*\).*?' +
        'SteamShell\.exe.*?CloseSteamShellProcessForSetup\(pid,\s*true\).*?' +
        'SteamShell-Helper\.exe.*?CloseSteamShellProcessForSetup\(pid,\s*false\)' -and
    $source -match
    ...
    $source -match
        '(?s)PollController\(\).*?ElevatedHelperOwnsForeground\(\).*?return' -and
    $source -match
        '(?s)RestartSteamShellInSafeMode\([^)]*\).*?' +
        '--steamshell-user-sid=.*?--steamshell-session-id=' -and
    $source -match
        '(?s)GetLinkedStandardUserToken\([^)]*\).*?' +
        'TokenElevationType.*?TokenLinkedToken.*?identity mismatch') (
    "Main/helper startup ownership or explicit administrator Setup takeover is disconnected.")
Assert-True (
    $helperSource -match '#NoTrayIcon' -and
    $helperSource -match '@Ahk2Exe-SetVersion 1\.9\.9\.1' -and
    $source -match
        'ElevatedHelperExpectedVersion\s*:=\s*"1\.9\.9\.1"' -and
    $helperSource -match 'if\s*!A_IsAdmin' -and
    $helperSource -match 'ProcessIntegrityRid\(pid\)' -and
    $helperSource -match 'ProcessIntegrityRid\(pid\)\s*>=\s*0x3000' -and
    $helperSource -match 'ParentPid\s*&&\s*!ProcessExist\(ParentPid\)' -and
    $helperSource -match
        '(?s)GetProcessIdentity\([^)]*\).*?ConvertSidToStringSidW.*?' +
        'ProcessIdToSessionId' -and
    $helperSource -match
        '(?s)GetProcessIdentity\(\s*selfPid.*?GetProcessIdentity\(\s*' +
    ...
    $helperSource -match
        '(?s)ExecuteBinding\(key\).*?SubStr\(value,\s*1,\s*5\)\s*=\s*"Send:".*?return' -and
...
Assert-True (
    $helperSource -match
        '(?s)LoadConfiguration\(\).*?EnableWindowManagement.*?' +
        'MinWidthPercent.*?ExcludeExeList.*?ExcludeClassList' -and
    $helperSource -match
        '(?s)ElevatedGeometryBuildItem\([^)]*\).*?' +
        'ProcessIntegrityRid\(pid\)\s*<\s*0x3000.*?' +
        'WmExcludeExeSet.*?WmExcludeClassSet.*?POPUP_CLASSES' -and
    $helperSource -match
        '(?m)^\s*return item\["x"\].*?item\["style"\]\s*$' -and
    $helperSource -match
        '(?s)ElevatedWindowGeometryTick\([^)]*\).*?' +
        'ParentAllowsElevatedGeometry.*?MAX_ATTEMPTS.*?' +
        'WinMove.*?WinMaximize.*?ElevatedGeometryState' -and
    $helperSource -match
        '(?s)OpenParentGeometryEvent\(\).*?' +
        'OpenEventW.*?Local\\SteamShellGeometry-.*?' +
        'ParentAllowsElevatedGeometry\(\).*?WaitForSingleObject' -and
    $helperSource -match
        'SetTimer\(ElevatedWindowGeometryTick,\s*500\)' -and
    $source -match
        '(?s)EnsureElevatedGeometryEvent\(\).*?CreateEventW' -and
    $source -match
        '(?s)SetElevatedGeometryRuntimeEnabled\(enabled\).*?' +
        'SetEvent.*?ResetEvent' -and
    $source -match
        '(?s)ApplyRuntimeTimers\(\).*?' +
        'SetElevatedGeometryRuntimeEnabled\(!DesktopMode\s*&&\s*!SafeMode\)' -and
    $source -match
        '(?s)WindowEngineGeometryHandledByHelper\(item\).*?' +
        'ElevatedHelperIsVerified.*?GetProcessTokenSecurity.*?' +
        'integrityName\s*=\s*"High".*?integrityName\s*=\s*"System"' -and
    $source -match
        '(?s)WindowEngineApplyGeometry\(snapshot\).*?' +
        'WindowEngineGeometryHandledByHelper\(item\).*?continue') (
    "Elevated geometry is not exclusively routed through the verified helper.")
Assert-True (
    $source -match
        '(?s)CreateProcessWithStandardToken\([^)]*\).*?' +
        'CreateEnvironmentBlock.*?STARTUPINFOW.*?CreateProcessWithTokenW.*?' +
        '"WStr",\s*executable.*?DestroyEnvironmentBlock' -and
    $source -match
        '(?s)GetLinkedStandardUserToken\([^)]*\).*?DuplicateTokenEx.*?' +
        'SecurityImpersonation.*?TokenPrimary' -and
    $source -match
        '(?s)CreateProcessWithStandardToken\([^)]*\).*?' +
        'EnableProcessTokenPrivilege\("SeImpersonatePrivilege".*?' +
        'Working directory is unavailable.*?"WStr",\s*directory' -and
    $source -match
        '(?s)LaunchInteractiveApp\([^)]*\).*?' +
        'GetLinkedStandardUserToken.*?CreateProcessWithStandardToken.*?' +
        'RunViaDesktopShell') (
    "The verified linked-token process launcher is incomplete.")
Assert-True (
    $source -match
        '(?s)GetTokenInformationBuffer\([^)]*&informationBuffer[^)]*\).*?' +
        'informationBuffer\s*:=\s*Buffer\(needed,\s*0\)' -and
    $source -notmatch
        '(?s)GetTokenInformationBuffer\([^)]*&buffer(?:\s|,|\))') (
