...
SteamShell **1.9.9** source, locked together with XFE in this release bundle;
1.7.3 remains the
pre-rendering-rewrite revert point. The feature-by-feature XFE port decisions are recorded in
`XFE_PARITY_NOTES.md`. The current settings schema is 18.
...
helper version and starts only that helper elevated. A Standard installation
registers a hidden, on-demand highest-privilege scheduled task whose action
points at the protected Program Files helper; this avoids a UAC prompt on every
shell start. If task registration or launch fails, SteamShell falls back to an
explicit UAC request. Portable/custom installs always use explicit UAC because
their executable directory cannot be assumed protected. The helper has no
...
geometry for elevated windows are unavailable. Helper activity and failures are recorded in
`logs\SteamShell-Helper.log` and reported by Health Check.
...
- **RTSS:** with `UseDllIntegration=true`, SteamShell reads RTSS's live global
  ...
  the number instead of forgetting it. **Save Limit to Profile** copies the current cap into the
...
`Build-SteamShell.ps1` runs `Validate-SteamShell.ps1`, validates and compiles
`SteamShell-Helper.ahk`, verifies helper version 1.9.9.1, and then validates and
compiles `SteamShell.ahk` with that helper embedded. It also verifies the
published main executable reports file version 1.9.9.0.
...
From this release bundle's root, double-click
`Run-SteamShellValidation.cmd` to validate
and build both SteamShell platforms together. The harness runs negative build
tests for both trees, checks both EXEs report file version 1.9.9.0, and only
after every check passes publishes them together to that folder's `current\`.
Failures return a non-zero exit code and leave the previous `current\` untouched.
