...
- Quick Menu → Steam Menu sends `Shift+Tab` and the row shows it.
- Tap **View**: nothing from the companion; the game still receives the button.
- Hold **View** past 1000 ms: the Steam overlay opens.
...
- In a real game, hold **View** while moving to check the overlay still
  triggers. Stick movement suppresses the action by design.
...
- Visit all seven categories and confirm nothing is clipped or overlapping. The
  window is 920x680; page content stops above the button bar at y630.

## Settings categories

- Confirm the sidebar lists seven categories, including the new **Steam**.
...
- Resolution and Refresh rate are two Left/Right rows; there is no paging.
- **Cycling must not change the display.** Only Apply does.
- Apply, do not confirm, and verify the automatic revert.
- Confirm the Apply row counts down (`Select to KEEP (12s)`) during the window.
- Repeat and select Apply again to keep the mode.
...
- **If the switcher is still dismissed, the default list does not name it.** Run
  **Quick Menu → System → Probe Screen**, open the switcher during the six-second
...
- Toggle HDR.
- Verify RTSS overlay and limiter shortcuts.
...
- **Press `Ctrl+Alt+Shift+I` (or Quick Menu → System → Re-arm Controller Input).**
...
- Set **Park the cursor at** to *Left edge* (`ParkEdge=left`), save, and confirm
  parking follows it both at startup and on the return to Steam.
