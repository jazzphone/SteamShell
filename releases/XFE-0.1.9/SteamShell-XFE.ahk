...
;
; Use Quick Menu -> System -> Probe Screen to identify anything else.
...
LoadSettings() {
    ...
    global ParkOnSteamReturn, ParkEdge, ParkYPercent, ForegroundPollMs
...
RawInputProbeMessage(wParam, lParam, msg, hwnd) {
    global EnableRawInputProbe, LearnActive
...
; ==============================================================================
; Display selection (resolution and refresh rate chosen independently)
; ==============================================================================
; Nothing here changes the display. The chosen pair is only applied when the
; Apply row is activated, so cycling through options is free and the existing
; 15-second revert still guards the single moment a mode actually changes.
...
ApplyDisplaySelection() {
    ...
    for index, mode in QuickMenuDisplayModes {
        if (mode["width"] = DisplaySelectedWidth
            ...
            SelectPrimaryDisplayMode(index)
            return
        }
    }
...
SelectPrimaryDisplayMode(modeIndex) {
    ...
    if ApplyPrimaryDisplayMode(candidate) {
        ...
        ; The selection is deliberately left alone. It previously jumped to the
        ; applied mode's row in the old paged list; on the fixed Display page
        ; that would move the cursor off Apply, which is the row the user needs
        ; to press again to keep the change.
...
RunStartupPrograms() {
    ...
    if (!EnableStartupPrograms || StartupPrograms.Length = 0)
        return
...
QuickMenuGetRows() {
    ...
    switch QuickMenuPage {
        ...
        case "DISPLAY":
            ; Resolution and refresh rate are independent choices, so they get a
            ; row each rather than being flattened into one list of every
            ; width x height x frequency combination. That list needed paging
            ; once a display reported more than seven modes, and cycling it
            ; applied a real mode change on every press.
            rows.Push(MenuRow("back", "Back", "", "back"))
        ...
        case "SYSTEM":
            ...
            rows.Push(MenuRow("screenProbe", "Probe Screen",
                "Log what is on screen in 6s", "screenProbe"))
            rows.Push(MenuRow("rearmInput", "Re-arm Controller Input",
                "After sleep", "rearmInput"))
...
; toggle from the Quick Menu writes the INI immediately and updates the live
; global, so it takes effect at once AND survives a restart -- the full Settings
; window's Save button is not involved and does not need to be.
...
QuickMenuToggleSetting(id) {
    ...
    try IniWrite(next ? "true" : "false", IniPath, meta["section"], meta["key"])
...
QuickMenuActivateSelected() {
    ...
    switch action {
        ...
        case "steamMenu":
            ...
            QuickMenuHideThenSend(SendSteamMenu(IsSteamProcess(QuickMenuPreviousExe)))
            return
        ...
        case "health":
            ...
            return
        case "screenProbe":
            ; Hide first: the probe is for identifying something else, and our
            ; own menu sitting in the foreground would be all it captured.
            HideQuickMenu()
...
ShowSettings(*) {
    ...
    control := settings.AddCheckbox("x300 y516 w320 h26", "Park after returning to Steam")
    ...
    label := settings.AddText("x300 y552 w170 h24 +0x200", "Park the cursor at")
    SettingsTrackControl("Controller & Cursor", label)
    parkEdgeDrop := settings.AddDropDownList("x480 y550 w150",
    ...
    control := settings.AddText("x300 y512 w570 h40 +Wrap",
        ...
        . "System → Probe Screen to identify an overlay that is not being caught.")
    SettingsTrackControl("Assist", control)
    ...
    settings.AddText("x24 y636 w430 h26 vSettingsStatus", "All changes saved")
    saveButton := settings.AddButton("x620 y630 w135 h34 Default", "Save && Apply")
    saveButton.OnEvent("Click", SaveSettings)
    closeButton := settings.AddButton("x770 y630 w110 h34", "Close")
    ...
    CenterGuiOnMonitorActual(settings, monitorIndex, 920, 680)
...
ShowHealthCheck(*) {
    ...
    global EnableRTSSIntegration, ControllerBackend, ActiveInputBackend
    global GameInputReady, GameInputFailed
