...
Assert-True (
    $source -match '(?s)CenterGuiOnMonitorActual\([^)]*\).*?'
        + 'WinGetPos\(&measuredX,\s*&measuredY,\s*&measuredWidth,\s*&measuredHeight' -and
...
Assert-True (
    $source -match '(?s)QuickMenuToggleSetting\([^)]*\)\s*\{.*?IniWrite\(') (
    "Quick Menu setting toggles must write the INI, not only the runtime global.")
Assert-True (
