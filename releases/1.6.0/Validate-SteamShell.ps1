...
Assert-True ($runtimeSchemaMatch.Groups[1].Value -eq "9") (
    "The feature-parity settings schema is not version 9.")
Assert-True (
...
Assert-True (
    ...
    $source -match
        ...
        '"rtssLimiterState".*?"rtssSettings".*?"RTSS Settings".*?return rows') (
...
Assert-True (
    $source -match
        '(?s)InitializeTrayMenu\(\)\s*\{.*?SteamShell\.ico.*?' +
        'Open Quick Menu.*?Exit SteamShell to Desktop') (
