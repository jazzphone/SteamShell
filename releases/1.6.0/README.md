...
`../releases/1.4.0`, and the coordinated window-engine baseline is preserved in
`../releases/1.5.0`. This tree is the locked SteamShell 1.6.0 source, preserved
in `../releases/1.6.0`. The feature-by-feature XFE port decisions are recorded
...
embeds the same multi-resolution icon in compiled builds. Its right-click menu
opens the Quick Menu, Full Settings, or Diagnostics; reloads settings; and exits
through SteamShell's guarded desktop-restoration path. A session Disable action
is intentionally not included because standalone SteamShell owns Explorer and
taskbar presentation while it is running.
