...
- Companion Disable/Enable. Pausing a utility companion is safe; pausing the
  standalone shell while it owns Explorer/taskbar state is not. The tray instead
  exits through the guarded desktop-restoration path.
...
The active tree is the locked `1.6.0` source with settings schema 9. Its frozen
checkpoint is `releases/1.6.0/`; the earlier `releases/1.5.0/` and
`releases/XFE-0.1.14/` directories remain unchanged.
