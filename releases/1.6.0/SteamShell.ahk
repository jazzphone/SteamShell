...
global SteamShellVersion := "1.6.0"
...
global CurrentSettingsSchemaVersion := 9
...
[SteamShell]
SettingsSchemaVersion=9                                    ; Internal schema used for safe settings upgrades
...
InitializeTrayMenu() {
 ...
 A_TrayMenu.Add("Exit SteamShell to Desktop", TrayExitToDesktop)
...
QuickMenuGetDefinitions() {
 ...
 if (QuickMenuPage = "RTSS") {
     ...
     if !rtssRunning {
         ...
         if IsObject(liveState) {
             ...
             rows.Push(Map("id", "rtssLimiterState", "label", "Frame Limiter"))
         } else {
 ...
 available := Map(
     ...
     "steammenu", Map("id", "steamMenu", "label", "Steam Menu"),
...
QuickMenuValue(id) {
 ...
 case "rtssOverlayState":
     ...
     return IsObject(state)
         ...
         : "Unavailable"
 case "rtssLimiterState":
     state := GetRtssGlobalState()
     return IsObject(state)
         ? "‹ " (state["limiter"] ? "ON" : "OFF") " ›  •  "
             . GetRtssFrameCapLabel()
         : "Unavailable"
 ...
 case "rtss":
     ...
     liveState := GetRtssGlobalState()
     if IsObject(liveState)
         return "‹ " (liveState["overlay"] ? "ON" : "OFF") " ›"
 ...
 case "framecap":
     ...
     liveState := GetRtssGlobalState()
     if IsObject(liveState)
         return "‹ " (liveState["limiter"] ? "ON" : "OFF") " ›  •  "
             . GetRtssFrameCapLabel()
...
QuickMenuAdjustSelected(direction) {
 ...
 case "rtssLimiterState":
     SetRtssFrameLimiterState(direction > 0)
 case "taskPrev", "taskNext":
     ChangeQuickMenuTaskPage(direction)
 case "qSplash", "qTaskbar", "qAudioControls", "qDisplayControls", "qControllerMouse", "qAutoHideCursor", "qParkBoot", "qParkFocus", "qParkEdge", "qSteamRefocus", "qGameAssist", "qAlwaysFocus", "qWindowManagement", "qFocusPaused", "qRtssIntegration", "qOverlayMode", "qLimiterMode", "qMouseSpeed", "qMouseHideDelay", "qFrameCap":
     AdjustQuickMenuSetting(id, direction)
 }
 QuickMenuRefresh()
}
...
QuickMenuActivateSelected() {
 ...
 case "rtssOverlayState", "overlayToggle":
     ToggleRtssOverlay()
 case "rtssLimiterState", "limiterToggle":
 ...
 case "qControllerMap":
     ...
     return
 case "qSplash", "qTaskbar", "qAudioControls", "qDisplayControls", "qControllerMouse", "qAutoHideCursor", "qParkBoot", "qParkFocus", "qParkEdge", "qSteamRefocus", "qGameAssist", "qAlwaysFocus", "qWindowManagement", "qFocusPaused", "qRtssIntegration", "qOverlayMode", "qLimiterMode":
     ToggleQuickMenuSetting(id)
...
SelectPrimaryDisplayMode(modeIndex) {
 ...
 DisplaySelectedWidth := candidate["width"]
 DisplaySelectedHeight := candidate["height"]
 DisplaySelectedFrequency := candidate["frequency"]
 EnsureDisplayScaleSelection()
 ApplyDisplaySelection()
}
...
GetRtssFrameCapLabel() {
 limit := GetRtssFrameLimit(GetQuickMenuPreviousExe())
 if IsObject(limit)
     return limit["fps"] > 0 ? limit["fps"] " FPS" : "Uncapped"
 return GetFrameCapLabel()
}
