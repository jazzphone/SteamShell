...
- **The readers went to `SteamShell-Shared.ahk`, not into standalone.** Both
  trees now compile one definition. That was not possible when this was written:
  XFE's readers could not strip an inline comment, and its `ReadBool` returned a
  hardcoded `false` for anything it did not recognise rather than the setting's
  own default. Porting them then would have imported two defects. Both were
  fixed first.
- **`CleanIniValue` semantics were the stated blocker and turned out to be free**,
  because that function had already moved to `SteamShell-Common.ahk`.
- Every migrated default and bound was compared before and after: **zero
  parameter changes, zero settings lost.**
- **One composed read survives, deliberately.** `EnableMouseParkOnFocusChange`
  takes its default from a retired key when one is present, so its default is a
...
The assertion that detected mismatched defaults is kept but can no longer fire,
so a second assertion now forbids the composed form outright. A rule that cannot
fail is the failure mode this project keeps rediscovering; the replacement is
