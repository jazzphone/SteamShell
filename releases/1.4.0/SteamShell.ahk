...
global SteamShellVersion := "1.4.0"
...
global CurrentSettingsSchemaVersion := 3
...
global QuickMenuMainOrderRaw := "Audio|Display|RTSS|Focus|Layout|Tasks|Game|Steam|Settings|System"
...
global QuickMenuDisplayModeIndex := 0
...
[SteamShell]
SettingsSchemaVersion=3                                    ; Internal schema used for safe settings upgrades
...
EnableMouseParkOnBoot=true                                  ; Move pointer off-screen once during startup
EnableMouseParkOnFocusChange=true                           ; Park once when SteamShell actually changes the focused window
EnableCursorHideOnBoot=true                                 ; Hide at startup without input when boot parking is disabled
EnableCursorHideOnRefocus=true                              ; Re-hide an already-idle cursor after SteamShell refocuses a window
...
MainOrder=Audio|Display|RTSS|Focus|Layout|Tasks|Game|Steam|Settings|System
...
WriteBoolIni(section, key, b) {
 return CommitIniChanges([
     Map("section", section, "key", key, "value", b ? "true" : "false")
 ])
}
...
LoadSettings() {
 ...
 EnableCursorHideOnBoot := ToBool(IniReadS("Features","EnableCursorHideOnBoot","true"), true)
 EnableCursorHideOnRefocus := ToBool(IniReadS("Features","EnableCursorHideOnRefocus","true"), true)
...
LauncherCleanupAudioPeakThreshold := ClampFloat(ToFloat(IniReadS("LauncherCleanup","AudioPeakThreshold","0.02"), 0.02), 0.0, 1.0)
 ...
 QuickMenuMainOrderRaw := IniReadS(
     "QuickMenu", "MainOrder",
     "Audio|Display|RTSS|Focus|Layout|Tasks|Game|Steam|Settings|System")
...
ApplyRuntimeTimers() {
 ...
 global PinnedForegroundHwnd, SettingsGui
 ...
 if (EnableSteamRefocusMode || EnableGameForegroundAssist || EnableAlwaysFocus || PinnedForegroundHwnd)
...
; ==============================================================================
; SHARED VISUAL THEME + LOG-ONLY ACTION MESSAGES
; ==============================================================================
ColorRefFromRgb(rgb) {
 r := (rgb >> 16) & 0xFF
 g := (rgb >> 8) & 0xFF
...
ApplyModernWindowStyle(guiObj, useBackdrop := false) {
 ; DWM calls are best-effort and simply do nothing on unsupported Windows builds.
 try {
     hwnd := guiObj.Hwnd
     dark := Buffer(4, 0)
     NumPut("Int", 1, dark, 0)
     ...
     corner := Buffer(4, 0)
     NumPut("Int", 2, corner, 0) ; DWMWCP_ROUND
     DllCall("dwmapi\DwmSetWindowAttribute", "Ptr", hwnd, "Int", 33, "Ptr", corner, "Int", 4)

     caption := Buffer(4, 0)
     NumPut("UInt", ColorRefFromRgb(0x101722), caption, 0)
     DllCall("dwmapi\DwmSetWindowAttribute", "Ptr", hwnd, "Int", 35, "Ptr", caption, "Int", 4)

     captionText := Buffer(4, 0)
     NumPut("UInt", ColorRefFromRgb(0xE8EEF7), captionText, 0)
     DllCall("dwmapi\DwmSetWindowAttribute", "Ptr", hwnd, "Int", 36, "Ptr", captionText, "Int", 4)

     if (useBackdrop) {
         backdrop := Buffer(4, 0)
         NumPut("Int", 2, backdrop, 0) ; DWMSBT_MAINWINDOW
         DllCall("dwmapi\DwmSetWindowAttribute", "Ptr", hwnd, "Int", 38, "Ptr", backdrop, "Int", 4)
     }
 }
}

ApplySteamShellTheme(guiObj, useBackdrop := false) {
 try guiObj.BackColor := "101722"
 try guiObj.SetFont("s10 cE8EEF7", "Segoe UI")
 ApplyModernWindowStyle(guiObj, useBackdrop)
}

ThemeNativeControl(ctrl) {
 ; Ask modern Windows builds to render compatible native controls in dark mode.
...
GetDefaultQuickMenuOrder() {
 return ["audio", "display", "rtss", "focus", "layout",
     "tasks", "game", "steam", "settings", "system"]
}
...
QuickMenuLayoutLabel(itemName) {
 ...
 case "rtss": return "RTSS + Frame Limiter"
 case "focus": return "Focus Assistance"
 case "layout": return "Controller Layout"
 case "tasks": return "Task Switcher"
 case "game": return "Return to Game"
 case "steam": return "Return to Steam"
...
QuickMenuGetDefinitions() {
 global QuickMenuPage, QuickMenuDisplayPage, QuickMenuDisplayModes
 ...
 if (QuickMenuPage = "AUDIO") {
     rows.Push(Map("id", "audioBack", "label", "Back to Quick Settings"))
     rows.Push(Map("id", "audio", "label", "Audio Output"))
 ...
 if (QuickMenuPage = "SETTINGS") {
     ...
     rows.Push(Map("id", "settingsFocus", "label", "Focus + Windows"))
 ...
 if (QuickMenuPage = "SETTINGS_RTSS") {
     ...
     rows.Push(Map("id", "qFrameCap", "label", "Configured Frame Cap"))
 ...
 if (QuickMenuPage = "DISPLAY") {
     rows.Push(Map("id", "displayBack", "label", "Back to Quick Settings"))
     ...
     pageSize := 8
     pageCount := Max(1, Ceil(QuickMenuDisplayModes.Length / pageSize))
     QuickMenuDisplayPage := ClampInt(QuickMenuDisplayPage, 1, pageCount)
     firstIndex := ((QuickMenuDisplayPage - 1) * pageSize) + 1
     lastIndex := Min(QuickMenuDisplayModes.Length, firstIndex + pageSize - 1)
     if (QuickMenuDisplayModes.Length = 0) {
         rows.Push(Map("id", "displayUnavailable", "label", "No display modes reported"))
     } else {
         Loop lastIndex - firstIndex + 1 {
             modeIndex := firstIndex + A_Index - 1
             mode := QuickMenuDisplayModes[modeIndex]
             rows.Push(Map(
                 "id", "displayMode:" modeIndex,
                 "label", mode["width"] " × " mode["height"]
             ))
         }
     }
     if (pageCount > 1) {
         rows.Push(Map("id", "displayPrev", "label", "Previous Page"))
         rows.Push(Map("id", "displayNext", "label", "Next Page"))
     }
     return rows
 }

 if (QuickMenuPage = "RTSS") {
     rows.Push(Map("id", "rtssBack", "label", "Back to Quick Settings"))
     if (RtssOverlayControlMode = "separate") {
         rows.Push(Map("id", "rtssOn", "label", "Overlay On"))
         rows.Push(Map("id", "rtssOff", "label", "Overlay Off"))
     } else {
         rows.Push(Map("id", "rtss", "label", "Toggle RTSS Overlay"))
     }
     if (RtssFrameLimiterControlMode = "separate") {
         rows.Push(Map("id", "framecapOn", "label", "Frame Limiter On"))
         rows.Push(Map("id", "framecapOff", "label", "Frame Limiter Off"))
     } else {
         rows.Push(Map("id", "framecap", "label", "Toggle Frame Limiter"))
     }
     return rows
 }
 ...
 available := Map(
     ...
     "rtss", Map("id", "rtssMenu", "label", "RTSS + Frame Limiter"),
     "focus", Map("id", "focus", "label", "Focus Assistance"),
     "layout", Map("id", "layout", "label", "Controller Layout"),
     "tasks", Map("id", "tasks", "label", "Task Switcher"),
     "game", Map("id", "game", "label", "Return to Game"),
     "steam", Map("id", "steam", "label", "Return to Steam"),
...
QuickMenuBuildGui() {
 global QuickMenuGui, QuickMenuRows, QuickMenuSelected, QuickMenuPage
 global SteamShellVersion, QuickMenuPreviousHwnd
 ...
 ApplySteamShellTheme(QuickMenuGui, true)
 QuickMenuGui.MarginX := 0
 QuickMenuGui.MarginY := 0

 defs := QuickMenuGetDefinitions()
 ; Keep the logical layout compact. AutoHotkey applies Windows DPI scaling to
 ; these values, then CenterGuiOnTargetMonitor uses the rendered pixel size.
 rowH := 36
 menuW := 600
 headerH := 72
 footerH := 44
 menuH := headerH + (defs.Length * rowH) + footerH + 12

 QuickMenuGui.SetFont("s18 Bold cF4F7FB", "Segoe UI")
 switch QuickMenuPage {
 case "AUDIO":
     title := "AUDIO"
     subtitle := "Output, volume, and mute controls"
 case "SYSTEM":
     title := "SYSTEM"
     subtitle := "Desktop, advanced settings, and protected power controls"
 case "SETTINGS":
     title := "SETTINGS"
     subtitle := "Controller-friendly options  •  Changes save automatically"
 case "SETTINGS_GENERAL":
     title := "GENERAL + STARTUP"
     subtitle := "Startup options take effect on the next launch"
 case "SETTINGS_INPUT":
     title := "CONTROLLER + CURSOR"
     subtitle := "A toggles  •  Left and right adjust values"
 case "SETTINGS_FOCUS":
     title := "FOCUS + WINDOWS"
     subtitle := "Persistent behavior and session focus controls"
 case "SETTINGS_RTSS":
     title := "RTSS + PERFORMANCE"
     subtitle := "Use the full editor for paths and shortcut recording"
 case "TASKS":
     title := "TASK SWITCHER"
     subtitle := "A switch + lock  •  Tap X close  •  Hold X force-close process"
 case "DISPLAY":
     title := "DISPLAY MODES"
     subtitle := "A applies  •  Unconfirmed changes revert after 15 seconds"
 case "RTSS":
     title := "RTSS SETTINGS"
     subtitle := "Configure RTSS shortcuts in SteamShellSettings.ini"
 case "LAYOUT":
     title := "CONTROLLER LAYOUT"
     subtitle := "Hold View/Back  •  Values show Short press / Long press"
 default:
     title := "QUICK SETTINGS"
     subtitle := "STEAMSHELL " SteamShellVersion "  •  Controller-first system controls"
 }
 QuickMenuGui.AddText("x26 y17 w548 h32", title)
 QuickMenuGui.SetFont("s9 Norm c8190A5", "Segoe UI")
 QuickMenuGui.AddText("x28 y47 w544 h18", subtitle)

 y := headerH
 for index, def in defs {
     QuickMenuGui.SetFont("s11 Norm cD5DCE8", "Segoe UI Semibold")
     if (QuickMenuPage = "LAYOUT") {
         labelOptions := "x30 y" y " w145 h" rowH " +0x200"
         valueOptions := "x180 y" y " w390 h" rowH " Right +0x200"
     } else if (QuickMenuPage = "TASKS") {
         labelOptions := "x30 y" y " w385 h" rowH " +0x200"
         valueOptions := "x420 y" y " w150 h" rowH " Right +0x200"
     } else {
         labelOptions := "x30 y" y " w280 h" rowH " +0x200"
         valueOptions := "x315 y" y " w255 h" rowH " Right +0x200"
     }
     labelCtrl := QuickMenuGui.AddText(labelOptions, def["label"])
     QuickMenuGui.SetFont("s10 Norm c93A4BB", "Segoe UI")
     valueCtrl := QuickMenuGui.AddText(valueOptions, "")
 ...
 if (QuickMenuPage = "LAYOUT")
     footerText := "Values show SHORT / LONG     •     B Back"
 else if (QuickMenuPage = "TASKS")
     footerText := "A Switch     Tap X Close     Hold X Force Close     B Back"
 else if (SubStr(QuickMenuPage, 1, 8) = "SETTINGS")
     footerText := "D-Pad Navigate     A Toggle/Open     B Back     ◀ ▶ Adjust"
 else
     footerText := "D-Pad Navigate     A Select     B Back     ◀ ▶ Adjust"
 QuickMenuGui.SetFont("s9 Norm c8190A5", "Segoe UI")
 QuickMenuGui.AddText("x28 y" (menuH-footerH+7) " w544 h24 Center"
     , footerText)

 try QuickMenuGui.Show("w" menuW " h" menuH)
 ApplyModernWindowStyle(QuickMenuGui, true)
 CenterGuiOnTargetMonitor(QuickMenuGui, QuickMenuPreviousHwnd)
 QuickMenuRefresh()
}
...
CenterGuiOnTargetMonitor(guiObj, targetHwnd := 0) {
 try {
     ...
     x := Round(workLeft + ((workRight - workLeft - guiW) / 2))
     y := Round(workTop + ((workBottom - workTop - guiH) / 2))
...
GetAudioMenuSummary() {
 ...
 if (outputName = "")
     return volumeText
 return volumeText "  •  " ShortenQuickMenuText(outputName, 22)
}
...
GetTaskSwitcherWindows() {
 ...
 for hwnd in WinGetList() {
     if (!hwnd || !DllCall("IsWindow", "Ptr", hwnd) || IsCloaked(hwnd))
         continue
     ...
     if (!(style & 0x10000000) || title = "" || pid = ScriptPid)
         continue
...
GetRtssMenuStatus() {
 ...
 if (RtssOverlayControlMode = "separate") {
     hasOverlay := RtssOverlayOnShortcut != "" && RtssOverlayOffShortcut != ""
     if (!hasOverlay && (RtssOverlayOnShortcut != "" || RtssOverlayOffShortcut != ""))
         return "OVERLAY INCOMPLETE"
 } else {
     hasOverlay := RtssOverlayToggleShortcut != ""
 }
 if (RtssFrameLimiterControlMode = "separate") {
     hasLimiter := RtssFrameLimiterOnShortcut != "" && RtssFrameLimiterOffShortcut != ""
     if (!hasLimiter && (RtssFrameLimiterOnShortcut != "" || RtssFrameLimiterOffShortcut != ""))
         return "LIMITER INCOMPLETE"
 } else {
     hasLimiter := RtssCustomFrameCapShortcut != ""
 }
 if (!hasOverlay && !hasLimiter)
     return "SHORTCUTS NEEDED"

 if ProcessExist("RTSS.exe")
     runtimeStatus := "RUNNING"
 else
     runtimeStatus := FileExist(NormalizeMediaPath(RtssPath)) ? "READY" : "NOT FOUND"

 if (runtimeStatus = "NOT FOUND")
     return "RTSS NOT FOUND"
 return "RTSS " runtimeStatus
}
...
GetDisplayModeMenuValue(modeIndex) {
 ...
 if (IsObject(current)
     ...
     && mode["frequency"] = current["frequency"]) {
     if IsObject(DisplayPendingOldMode) {
         seconds := Max(0, Ceil((DisplayPendingUntilTick - A_TickCount) / 1000))
         return "KEEP?  " seconds "s"
     }
     return "CURRENT  •  " mode["frequency"] " Hz"
 }
 return mode["frequency"] " Hz"
}
...
QuickMenuValue(id) {
 ...
 if (SubStr(id, 1, 12) = "displayMode:")
     return GetDisplayModeMenuValue(ToInt(SubStr(id, 13), 0))
 ...
 case "displayPrev", "displayNext":
     pageCount := Max(1, Ceil(QuickMenuDisplayModes.Length / 8))
     return QuickMenuDisplayPage " / " pageCount
 ...
 case "rtss":
     ...
     if (RtssOverlayToggleShortcut = "")
         return "SHORTCUT NEEDED"
     if (!ProcessExist("RTSS.exe") && !FileExist(NormalizeMediaPath(RtssPath)))
         return "RTSS NOT FOUND"
     return "TOGGLE"
 case "rtssOn":
     if (!EnableRTSSIntegration)
         return "SETUP REQUIRED"
     if (RtssOverlayOnShortcut = "")
         return "SHORTCUT NEEDED"
     if (!ProcessExist("RTSS.exe") && !FileExist(NormalizeMediaPath(RtssPath)))
         return "RTSS NOT FOUND"
     return "SHOW"
 ...
 case "framecap":
     ...
     if (!ProcessExist("RTSS.exe") && !FileExist(NormalizeMediaPath(RtssPath)))
         return "RTSS NOT FOUND"
     return RtssCustomFrameCap > 0 ? ("TOGGLE  •  " RtssCustomFrameCap " FPS") : "TOGGLE"
 case "framecapOn":
     if (!EnableRTSSIntegration)
         return "SETUP REQUIRED"
     if (RtssFrameLimiterOnShortcut = "")
         return "SHORTCUT NEEDED"
     if (!ProcessExist("RTSS.exe") && !FileExist(NormalizeMediaPath(RtssPath)))
         return "RTSS NOT FOUND"
     return RtssCustomFrameCap > 0 ? ("ENABLE  •  " RtssCustomFrameCap " FPS") : "ENABLE"
 case "framecapOff":
     if (!EnableRTSSIntegration)
         return "SETUP REQUIRED"
     if (RtssFrameLimiterOffShortcut = "")
         return "SHORTCUT NEEDED"
     if (!ProcessExist("RTSS.exe") && !FileExist(NormalizeMediaPath(RtssPath)))
         return "RTSS NOT FOUND"
     return "DISABLE"
 case "focus":
     return FocusAssistancePaused ? "PAUSED" : "ON"
 case "qSplash":
 ...
 case "qWindowManagement":
     return EnableWindowManagement ? "ON" : "OFF"
 case "qFocusPaused":
 ...
 case "layout", "system", "settings", "settingsGeneral", "settingsInput", "settingsFocus", "settingsRtss", "settingsEditor", "qControllerMap":
     return "›"
...
QuickMenuRefresh() {
 ...
 for index, row in QuickMenuRows {
     ...
     prefix := selected ? "›  " : "   "
     row["labelCtrl"].Text := prefix row["label"]
     row["valueCtrl"].Text := QuickMenuValue(row["id"])
     if (selected) {
         try row["labelCtrl"].SetFont("s11 Bold c66C0F4", "Segoe UI Semibold")
         try row["valueCtrl"].SetFont("s10 Bold c66C0F4", "Segoe UI")
     } else {
         try row["labelCtrl"].SetFont("s11 Norm cD5DCE8", "Segoe UI Semibold")
         try row["valueCtrl"].SetFont("s10 Norm c93A4BB", "Segoe UI")
     }
 }
}
...
QuickMenuAdjustSelected(direction) {
 ...
 case "displayPrev", "displayNext":
     ChangeQuickMenuDisplayPage(direction)
 case "taskPrev", "taskNext":
     ChangeQuickMenuTaskPage(direction)
 case "qSplash", "qTaskbar", "qAudioControls", "qDisplayControls", "qControllerMouse", "qAutoHideCursor", "qParkBoot", "qParkFocus", "qSteamRefocus", "qGameAssist", "qAlwaysFocus", "qWindowManagement", "qFocusPaused", "qRtssIntegration", "qOverlayMode", "qLimiterMode", "qMouseSpeed", "qMouseHideDelay", "qFrameCap":
     AdjustQuickMenuSetting(id, direction)
 }
 QuickMenuRefresh()
}
...
OpenQuickMenuDisplayPage() {
 global QuickMenuPage, QuickMenuSelected, QuickMenuDisplayPage, QuickMenuDisplayModes
 QuickMenuDisplayModes := GetPrimaryDisplayModes()
 ...
 current := GetPrimaryDisplayMode()
 if IsObject(current) {
     for index, mode in QuickMenuDisplayModes {
         if (mode["width"] = current["width"] && mode["height"] = current["height"]
             && mode["frequency"] = current["frequency"]) {
             QuickMenuDisplayPage := Ceil(index / 8)
             QuickMenuSelected := 3 + Mod(index - 1, 8)
             break
         }
     }
 }
...
ChangeQuickMenuDisplayPage(direction) {
 global QuickMenuDisplayPage, QuickMenuDisplayModes, QuickMenuSelected
 pageCount := Max(1, Ceil(QuickMenuDisplayModes.Length / 8))
 QuickMenuDisplayPage += direction
 if (QuickMenuDisplayPage < 1)
     QuickMenuDisplayPage := pageCount
 if (QuickMenuDisplayPage > pageCount)
     QuickMenuDisplayPage := 1
 QuickMenuSelected := 1
 QuickMenuBuildGui()
}
...
SelectTaskSwitcherWindow(hwnd) {
 global PinnedForegroundHwnd, PinnedForegroundTitle, PinnedForegroundExe
 global SteamRefocusPollingInterval
 ...
 if ActivateWindowRobust(hwnd) {
     ...
     ShowNotification("Focus lock: " ShortenQuickMenuText(item["title"], 36), "Success")
 } else {
     ReleasePinnedForeground(false)
...
QuickMenuActivateSelected() {
 ...
 global FocusAssistancePaused, BpmTitle, QuickMenuPreviousHwnd
 ...
 if (SubStr(id, 1, 12) = "displayMode:") {
     SelectPrimaryDisplayMode(ToInt(SubStr(id, 13), 0))
     return
 }
 if (SubStr(id, 1, 7) = "layout:")
     return
 ...
 case "hdr":
     SendChordSafe("#!b")
     ShowNotification("Windows HDR toggle requested", "Success")
 case "display":
     OpenQuickMenuDisplayPage()
     return
 case "displayPrev":
     ChangeQuickMenuDisplayPage(-1)
     return
 case "displayNext":
     ChangeQuickMenuDisplayPage(1)
     return
 ...
 case "qControllerMap":
     ...
     return
 case "qSplash", "qTaskbar", "qAudioControls", "qDisplayControls", "qControllerMouse", "qAutoHideCursor", "qParkBoot", "qParkFocus", "qSteamRefocus", "qGameAssist", "qAlwaysFocus", "qWindowManagement", "qFocusPaused", "qRtssIntegration", "qOverlayMode", "qLimiterMode":
     ToggleQuickMenuSetting(id)
 case "qMouseSpeed", "qMouseHideDelay", "qFrameCap":
     AdjustQuickMenuSetting(id, 1)
 case "focus":
     FocusAssistancePaused := !FocusAssistancePaused
     ShowNotification(FocusAssistancePaused ? "Focus assistance paused" : "Focus assistance resumed", "Success")
 case "game":
     ReleasePinnedForeground(false)
     hwnd := QuickMenuPreviousHwnd
     HideQuickMenu(false)
     if (hwnd && DllCall("IsWindow", "Ptr", hwnd)) {
         ...
         ForceGameAssistOnce()
         ShowNotification("Asked SteamShell to find the active game")
     }
 case "steam":
     ReleasePinnedForeground(false)
     HideQuickMenu(false)
     hwnd := WinExist(BpmTitle)
     if (hwnd) {
         if ActivateWindowRobust(hwnd) {
             HandleCursorAfterManagedFocus(hwnd, false)
         }
     }
     ShowNotification(hwnd ? "Returned to Steam" : "Steam Big Picture was not found", hwnd ? "Success" : "Warning")
 case "layout":
...
SelectPrimaryDisplayMode(modeIndex) {
 global QuickMenuDisplayModes, QuickMenuDisplayModeIndex, QuickMenuDisplayPage, QuickMenuSelected
 global DisplayPendingOldMode, DisplayPendingUntilTick
 ...
 candidateIsCurrent := (candidate["width"] = current["width"]
 ...
 if IsObject(DisplayPendingOldMode) {
     if (candidateIsCurrent)
         ...
         ShowNotification("Keep or revert the current display change first", "Warning")
     return
 }
 ...
 if ApplyPrimaryDisplayMode(candidate) {
     ...
     QuickMenuDisplayModeIndex := modeIndex
     QuickMenuDisplayPage := Ceil(modeIndex / 8)
     QuickMenuSelected := 3 + Mod(modeIndex - 1, 8)
     SetTimer(DisplayChangeSafetyTick, 500)
     QuickMenuBuildGui()
     ShowNotification("Display changed. Select the marked mode again to keep it.", "Warning")
 } else {
     ShowNotification("Windows rejected that display mode", "Warning")
 }
}

CyclePrimaryDisplayMode(direction) {
 global QuickMenuDisplayModes, QuickMenuDisplayModeIndex
 global DisplayPendingOldMode, DisplayPendingUntilTick
 ...
 if (QuickMenuDisplayModes.Length = 0)
     QuickMenuDisplayModes := GetPrimaryDisplayModes()
 if (QuickMenuDisplayModes.Length = 0) {
     ShowNotification("Windows reported no compatible display modes", "Warning")
     return
 }
 ...
 nextIndex := currentIndex + direction
 if (nextIndex < 1)
     nextIndex := QuickMenuDisplayModes.Length
 if (nextIndex > QuickMenuDisplayModes.Length)
     nextIndex := 1

 candidate := QuickMenuDisplayModes[nextIndex]
 if ApplyPrimaryDisplayMode(candidate) {
     DisplayPendingOldMode := current
     DisplayPendingUntilTick := A_TickCount + 15000
     QuickMenuDisplayModeIndex := nextIndex
     SetTimer(DisplayChangeSafetyTick, 500)
     ShowNotification("Display changed. Select Display Mode again to keep it.", "Warning")
 } else {
     ShowNotification("Windows rejected that display mode", "Warning")
 }
 QuickMenuRefresh()
...
ConfirmPrimaryDisplayMode() {
 ...
 ShowNotification("Display mode kept", "Success")
 QuickMenuRefresh()
}
...
DisplayChangeSafetyTick() {
 ...
 if (A_TickCount >= DisplayPendingUntilTick) {
     ...
     if ApplyPrimaryDisplayMode(oldMode)
         ShowNotification("Display mode reverted for safety", "Warning")
     else
         ShowNotification("Unable to restore the previous display mode", "Warning")
 }
 QuickMenuRefresh()
}
...
PollController() {
 ...
 ; Controller chord: LT+RT+LB+RB+L3+R3 opens Full Settings (works even without holding View/Back).
 static settingsComboWasDown := false
 settingsComboNow := ((lt > 30) && (rt > 30) && (buttons & 0x0100) && (buttons & 0x0200) && (buttons & 0x0040) && (buttons & 0x0080))
 if (settingsComboNow && !settingsComboWasDown) {
 try ShowSettingsEditor()
 }
 settingsComboWasDown := settingsComboNow
 ...
 if (settingsControllerActive) {
     if (SettingsEditorDialogActive || !(buttons & 0x0020)) {
         ...
         downTick["LT"] := 0, longFired["LT"] := false, prevTrigDown["LT"] := false
         downTick["RT"] := 0, longFired["RT"] := false, prevTrigDown["RT"] := false
         SettingsEditorHandleController(pressed, lx, ly, rx, ry, rt)
         return
     }
 }
...
MonitorShell() {
 ...
 static startupWarningLogged := false
 ...
 if (steamRunning) {
     ...
     if (!SteamObservedRunning) {
         try LogLine("Shell monitor: Steam process observed; automatic exit monitoring is now armed.")
         SetSessionState("ACTIVE", "Steam observed")
     }
     SteamObservedRunning := true
     SteamMissingSinceTick := 0
     startupWarningLogged := false
     ...
     if (!SteamObservedRunning) {
         ...
         if (SteamLaunchAttemptTick > 0
             ...
             && !startupWarningLogged) {
             startupWarningLogged := true
             try LogLine(
                 "Shell monitor: Steam was not observed within the startup grace period; "
                 . "keeping SteamShell active and automatic desktop restoration disarmed.")
     ...
     } else if (SteamMissingSinceTick = 0) {
...
TryBringFullscreenCandidateToFront(forceRun := false, forceActivate := false) {
 ...
 wasAlreadyActive := WinActive("ahk_id " best["hwnd"]) != 0
 ok := ActivateWindowRobust(best["hwnd"])
 LastGameBringToFrontTick := A_TickCount
 if (ok) {
     HandleCursorAfterManagedFocus(best["hwnd"], wasAlreadyActive)
 }
...
; ==============================================================================
MaintainPinnedForeground() {
 global PinnedForegroundHwnd, ScriptPid, LastActionText
 if (!PinnedForegroundHwnd)
     return false
 if (!DllCall("IsWindow", "Ptr", PinnedForegroundHwnd) || IsCloaked(PinnedForegroundHwnd)) {
     ReleasePinnedForeground(false)
     return false
 }
 ...
 if (activeHwnd = PinnedForegroundHwnd)
     return true

 ; Let SteamShell's own menus stay usable and allow dialogs owned by the
 ; selected process without treating them as a focus loss.
 if (activeHwnd) {
     try {
         activePid := WinGetPID("ahk_id " activeHwnd)
         pinnedPid := WinGetPID("ahk_id " PinnedForegroundHwnd)
         if (activePid = ScriptPid || activePid = pinnedPid)
             return true
     }
 }

 ok := ActivateWindowRobust(PinnedForegroundHwnd)
 if (ok) {
     HandleCursorAfterManagedFocus(PinnedForegroundHwnd, false)
 }
 LastActionText := ok ? "Task focus lock: restored" : "Task focus lock: activation failed"
 return true
}
...
SteamRefocusPolling() {
 ...
 wasAlreadyActive := WinActive("ahk_id " best) != 0
 try WinActivate("ahk_id " best)
 HandleCursorAfterManagedFocus(best, wasAlreadyActive)
 ...
 wasAlreadyActive := WinActive("ahk_id " hBpm) != 0
 try WinActivate("ahk_id " hBpm)
 HandleCursorAfterManagedFocus(hBpm, wasAlreadyActive)
...
AF_SelectExecutable(prompt) {
 global AlwaysFocusGui, SettingsGui
 if !IsSet(AlwaysFocusGui)
     return ""
...
ForceSteamFocusNow(*) {
 ...
 wasAlreadyActive := WinActive("ahk_id " h) != 0
 try WinActivate("ahk_id " h)
 HandleCursorAfterManagedFocus(h, wasAlreadyActive)
...
WriteToIniFromGui(*) {
 ...
 if (ControlGui["cbLiveLog"].Value = 1) {
 changes.Push(Map("section", "Logging", "key", "EnableGameScoreLogging", "value", "true"))
 ...
 changes.Push(Map("section", "Logging", "key", "EnableGameScoreLogging",
     "value", EnableGameScoreLogging ? "true" : "false"))
...
SettingsEditorAddTextField(category, section, key, label, &y, defaultValue := ""
 ...
 global SettingsGui, SettingsEditorFields
 labelCtrl := SettingsGui.AddText("x255 y" (y + 4) " w225 h24", label)
 options := "x485 y" y " w460"
 if (rows > 1)
     options .= " r" rows " WantTab"
 ctrl := SettingsGui.AddEdit(options, IniReadS(section, key, defaultValue))
...
SettingsEditorAddChoice(category, section, key, label, choices, &y, defaultValue := "") {
 global SettingsGui, SettingsEditorFields
 labelCtrl := SettingsGui.AddText("x255 y" (y + 4) " w225 h24", label)
 ctrl := SettingsGui.AddDropDownList("x485 y" y " w260", choices)
...
SettingsEditorAddShortcutField(category, section, key, label, &y, defaultValue := "") {
 global SettingsGui, SettingsEditorFields
 labelCtrl := SettingsGui.AddText("x255 y" (y + 4) " w225 h24", label)
 ctrl := SettingsGui.AddEdit("x485 y" y " w335", IniReadS(section, key, defaultValue))
...
SettingsEditorControllerActive() {
 ...
 if (SettingsEditorDialogActive) {
     try {
         ...
         return WinGetPID("ahk_id " activeHwnd) = ScriptPid
             && WinGetClass("ahk_id " activeHwnd) = "#32770"
     }
 }
 try return IsGuiVisible(SettingsGui) && WinActive("ahk_id " SettingsGui.Hwnd)
 return false
}
...
SettingsEditorAdjustFocusedControl(direction) {
 ...
 if (fieldType = "integer" && !RegExMatch(currentValue, "^-?\d+$"))
     return
 if (fieldType = "float" && !RegExMatch(currentValue, "^-?(?:\d+(?:\.\d*)?|\.\d+)$"))
     return
 step := 1
 if (fieldType = "float")
     step := 0.05
...
SettingsEditorHandleController(pressed, lx := 0, ly := 0, rx := 0, ry := 0, rt := 0) {
 global SettingsEditorDirty, SettingsEditorStatusCtrl, SettingsEditorDialogActive
 global ControllerMouseSpeed, ControllerMouseFastMultiplier
 static stickDir := ""
 static lastStickTick := 0

 ; Settings is a couch-facing window, so the right stick always moves the
 ; mouse here. Hold View/Back to use the user's normal mapped click actions.
 if (rx != 0 || ry != 0) {
     speed := ControllerMouseSpeed
     if (rt > 30)
         speed := Round(speed * ControllerMouseFastMultiplier)
 ...
 if (pressed & 0x0100) { ; LB
     SettingsEditorSwitchControllerCategory(-1)
     return
 }
 if (pressed & 0x0200) { ; RB
     SettingsEditorSwitchControllerCategory(1)
     return
 }
...
GetSteamShellHealthResults() {
 ...
 HealthResult(results, taskbarHwnd ? "pass" : "warn", "Desktop taskbar",
     taskbarHwnd ? "Windows reports a taskbar window." : "No Shell_TrayWnd was found.")
...
ShowSettingsEditor(*) {
 ...
 SettingsGui := Gui(
     "+AlwaysOnTop +ToolWindow +Resize -MaximizeBox +MinSize980x450 +MaxSize980x2000",
 ...
 windowDpi := A_ScreenDPI
 dpiSourceHwnd := SettingsGui.Hwnd
 if (LastRealFgHwnd && DllCall("IsWindow", "Ptr", LastRealFgHwnd))
     dpiSourceHwnd := LastRealFgHwnd
 try windowDpi := DllCall("User32\GetDpiForWindow", "Ptr", dpiSourceHwnd, "UInt")
 if (windowDpi <= 0)
     windowDpi := 96
 availableLogicalHeight := Floor(((workBottom - workTop) - 32) * 96 / windowDpi)
 SettingsEditorWindowHeight := ClampInt(availableLogicalHeight, 450, 800)
 ...
 SettingsGui.AddText("x22 y49 w930 h22"
     , "D-pad navigate • A activate • Right stick mouse • Hold Back for mapped controls • Y save")
 ...
 SettingsEditorAddCheckbox(category, "Features", "EnableWindowManagement", "Center and maximize eligible application windows", &y, "true")
 ...
 SettingsEditorAddHeading(category, "Startup & Splash"
     , "Executable paths use Windows file pickers. Startup and splash changes take effect on the next launch.")
 y := 150
 ...
 SettingsEditorAddTextField(category, "Controller", "ControllerDeadzone", "Stick deadzone", &y, "8000", "integer", 0, 32000)
 SettingsEditorAddTextField(category, "Controller", "ControllerPollIntervalMs", "Controller polling interval (ms)", &y, "16", "integer", 5, 200)
 ...
 SettingsEditorAddCheckbox(category, "Features", "EnableMouseParkOnBoot", "Park the mouse off-screen once during startup", &y, "true")
 SettingsEditorAddCheckbox(category, "Features", "EnableMouseParkOnFocusChange", "Park once after a managed focus change", &y, "true")
 SettingsEditorAddCheckbox(category, "Features", "EnableCursorHideOnBoot", "Hide the cursor on boot when parking is disabled", &y, "true")
 ...
 SettingsEditorAddHeading(category, "Focus & Windows"
     , "Control when SteamShell restores Steam, promotes a game, or manages normal application windows.")
 y := 150
 ...
 SettingsEditorAddCheckbox(category, "Features", "EnableGameForegroundAssist", "Bring the active fullscreen-style game forward", &y, "true")
 ...
 SettingsEditorAddTextField(category, "GameForegroundAssist", "GameForegroundCooldownMs", "Game focus cooldown (ms)", &y, "1500", "integer", 0, 60000)
 SettingsEditorAddCheckbox(category, "Features", "EnableAlwaysFocus", "Enable the AlwaysFocus executable list", &y, "true")
 SettingsEditorAddTextField(category, "AlwaysFocus", "AlwaysFocusCooldownMs", "AlwaysFocus cooldown (ms)", &y, "1000", "integer", 0, 60000)
 SettingsEditorAddTextField(category, "WindowManagement", "MinWidthPercent", "Minimum width fraction for maximizing", &y, "0.20", "float", 0.05, 1.0)
 exclusionY := y + 8
 ...
 SettingsEditorAddHeading(category, "RTSS & Performance"
     , "Select RTSS.exe and use Record for matching HotkeyHandler shortcuts. Fields remain editable for expert use.")
 y := 150
 ...
 SettingsEditorAddChoice(category, "RTSS", "OverlayControlMode", "Overlay control mode", ["Toggle", "Separate"], &y, "Toggle")
 ...
 SettingsEditorAddChoice(category, "RTSS", "FrameLimiterControlMode", "Frame limiter control mode", ["Toggle", "Separate"], &y, "Toggle")
 SettingsEditorAddTextField(category, "RTSS", "CustomFrameCap", "Configured frame cap (FPS)", &y, "0", "integer", 0, 1000)
 ...
 SettingsEditorAddCheckbox(category, "LauncherCleanup", "RequireNoGame", "Require SteamShell to detect that no game is running", &y, "true")
 ...
 SettingsEditorAddCheckbox(category, "LauncherCleanup", "UseCpuAudio", "Use CPU and audio activity for the no-game gate", &y, "true")
 SettingsEditorAddTextField(category, "LauncherCleanup", "CpuThreshold", "CPU activity threshold", &y, "12", "integer", 0, 500)
 SettingsEditorAddCheckbox(category, "LauncherCleanup", "DownloadGuard", "Skip cleanup during possible downloads or updates", &y, "false")
 ...
 y := 150
 SettingsEditorAddCheckbox(category, "Logging", "EnableGameScoreLogging", "Enable game foreground scoring logs", &y, "false")
 SettingsEditorAddChoice(category, "Logging", "GameLogMode", "Game log detail", ["OFF", "ACTIVATIONS", "TOPN", "DIAGNOSTIC"], &y, "OFF")
 ...
 actionY := y + 12
 SettingsEditorAddActionButton(category, "Pause / Resume Focus", SettingsEditorToggleFocusPause, 255, actionY, 205)
 SettingsEditorAddActionButton(category, "Run Game Assist", SettingsEditorRunGameAssist, 470, actionY, 180)
 SettingsEditorAddActionButton(category, "Reload Runtime", SettingsEditorReloadRuntime, 660, actionY, 180)
 SettingsEditorAddActionButton(category, "Open INI in Notepad", SettingsEditorOpenIni, 255, actionY + 38, 205)
 SettingsEditorAddActionButton(category, "Open Live Log", ShowLiveLogWindow, 470, actionY + 38, 180)
 SettingsEditorAddActionButton(category, "Diagnostics Panel…", ShowControlPanel, 660, actionY + 38, 180)
 SettingsEditorAddActionButton(
     category, "Install SteamShell as Shell", SettingsEditorInstallSteamShell, 255, actionY + 76, 220)
 SettingsEditorAddActionButton(
     category, "Repair SteamShell Installation", SettingsEditorRepairSteamShell, 485, actionY + 76, 220)
 SettingsEditorAddActionButton(
     category, "Permanently Restore Explorer", SettingsEditorRestoreDesktop, 715, actionY + 76, 230)
 SettingsEditorAddActionButton(category, "Health Check…", ShowHealthCheck, 255, actionY + 114, 205)
 SettingsEditorAddActionButton(category, "Export Diagnostic ZIP", ExportDiagnosticBundle, 470, actionY + 114, 180)
 SettingsEditorAddActionButton(category, "Create Settings Backup", SettingsEditorCreateBackup, 660, actionY + 114, 220)
 SettingsEditorAddActionButton(category, "Export Settings…", SettingsEditorExportSettings, 255, actionY + 152, 205)
 SettingsEditorAddActionButton(category, "Import / Restore…", SettingsEditorImportSettings, 470, actionY + 152, 180)
 SettingsEditorAddActionButton(category, "Restore Category Defaults…", SettingsEditorResetCategory, 660, actionY + 152, 220)
 SettingsEditorAddActionButton(category, "Reset All Settings…", SettingsEditorResetAll, 255, actionY + 190, 205)
 SettingsEditorAddActionButton(category, "Restart in Safe Mode", RestartSteamShellInSafeMode, 470, actionY + 190, 180)
 SettingsEditorAddActionButton(category, "Setup Assistant…", ShowSetupAssistant, 660, actionY + 190, 220)
...
PrepareForDesktopRestore() {
 global AllowExplorer, QuickMenuVisible
 AllowExplorer := true
 SystemCursor("Show")
 ReleasePinnedForeground(false)
...
ExitSteamAndRestoreDesktop() {
 ...
 if ProcessExist("steam.exe") {
     ShowNotification("Steam did not close. Desktop restore was cancelled.", "Warning")
     return
 }
...
ExitToDesktop(PermanentRestore := false) {
 ...
 if (result = "Retry")
     ExitToDesktop(PermanentRestore)
}
...
RunHiddenCommandLine(cmdline) {
 return RunStartupCommandLine(cmdline, "Hidden")
}
