...
- Cursor helpers: park the pointer off-screen once at boot and once when SteamShell actually changes focus to a
  managed window, then keep it hidden without generating recurring input. Periodic checks of an already-focused
  window do not move the pointer, so Windows' display-off and sleep timers can still expire. Physical mouse movement
  restores the cursor.
...
- **Focus + Windows:** Steam refocus, game foreground assistance, AlwaysFocus support, window management, and the
  session focus pause.
...
between controls, left/right adjusts choices and numeric values, A activates the selected control, LB/RB changes
category, Y saves, and B closes when no changes are pending. The right stick automatically controls the pointer
inside Settings. Holding View/Back temporarily uses the normal configurable mappings, including mapped mouse
clicks and shortcuts. Selecting an edit field with A opens the Windows touch keyboard; unsaved changes are never
silently discarded from the controller.
...
`/selftest` runs parser, schema, list-normalization, Quick Menu ordering, and startup-command invariants without
starting the normal shell session.
...
functions, ampersand-safe Settings headings, and whitespace hygiene.
...
SteamShell remains an AutoHotkey v2.0.19 64-bit script and compiles into one executable with Ahk2Exe. On Windows,
install AutoHotkey v2 with its compiler and run:
