...
The working tree is **0.1.21**. The last locked source snapshot is
`releases/XFE-0.1.21`. Controller input, the Quick Menu, Settings,
...
Windows-shell application, whose working tree is 1.7.7.

0.1.21 advances settings schema to 8. Every companion-owned settings/editor
...
hardening described above; the current 0.1.21 working tree uses schema 8. Use
...
From the repository root, `Run-SteamShellValidation.cmd` validates and builds
both SteamShell platforms, exercises broken-source and stale-output rejection in
both trees, and publishes both verified EXEs together to root `current\` only
after the complete run passes. A failed run returns non-zero and preserves the
