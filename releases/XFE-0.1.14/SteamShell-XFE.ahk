...
; Retained as a compatibility entry point for validation and future callers.
; It feeds the same combined transaction rather than creating a second display
; apply path that could forget to preserve or restore scale.
        ...
        case "RTSS":
            ...
            if !rtssRunning {
                ...
                if IsObject(rtssState) {
                    ...
                    rows.Push(MenuRow("rtssLimiterState", "Frame Limiter",
                        "‹ " (rtssState["limiter"] ? "ON" : "OFF") " ›  •  "
                            . GetRtssFrameCapLabel(QuickMenuPreviousExe),
                        "rtssLimiterState", true))
...
GetRtssFrameCapLabel(profileExe := "") {
    limit := GetRtssFrameLimit(profileExe)
    if !IsObject(limit)
