...
Assert-True (
    ...
    $source -match '(?s)case "RTSS":.*?MenuRow\("rtssOverlayState", "Overlay".*?MenuRow\("rtssLimiterState", "Frame Limiter"') (
...
Assert-True (
    $source -match '(?s)QuickMenuActivateSelected\(\).*?case "rtssOverlayState":\s*\r?\n\s*ToggleRtssOverlay\(\).*?case "rtssLimiterState":\s*\r?\n\s*ToggleRtssFrameLimiter\(\)' -and
    $source -match '(?s)QuickMenuAdjustSelected\([^)]*\).*?case "rtssOverlayState":\s*\r?\n\s*SetRtssOverlayState\(direction > 0\).*?case "rtssLimiterState":\s*\r?\n\s*SetRtssFrameLimiterState\(direction > 0\)') (
    "RTSS state rows must support A toggle and explicit Left/Right Off/On.")
Assert-True (
