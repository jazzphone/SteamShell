...
The current release is **0.1.14**, locked as a reproducible source snapshot in
`releases/XFE-0.1.14`. Controller input, the Quick Menu, Settings, display/HDR
controls, RTSS integration, and notification-area control work inside Xbox FSE.
The companion remains deliberately separate from the stable SteamShell 1.5
...
**RTSS & Performance** reads RTSS's global Overlay and Frame Limiter flags from
`RTSSHooks64.dll` when RTSS is running. Each row shows **On** or **Off**; A
toggles it, Left explicitly turns it off, and Right turns it on. The limiter row
also reads `FramerateLimit` from the RTSS profile for the application that was
in front when the Quick Menu opened; RTSS supplies the global profile when no
application-specific profile exists. It shows the actual value, such as
**117 FPS**, or **Uncapped** for zero.
...
process is active—because Overlay and Limiter already have dedicated rows there.
