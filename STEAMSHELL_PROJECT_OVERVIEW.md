...
| `SteamShell/SteamShell-Shared.ahk` | The **tree-coupled** half: RTSS orchestration, Quick Menu painting, elevated-helper lifecycle. `#Include`d by both trees only, because it reaches into per-tree functions and would not compile in the helper. Functions only — no top-level code, because it is inserted into two different auto-execute sections. Calls back into the per-tree functions enumerated in `$sharedSeamAllowed` (`Validate-Common.ps1`) — the seam, whose size is asserted against `$sharedSeamExpectedCount` rather than written down here, because this sentence has carried a wrong count once already — of which the `Product*` group (`ProductIdentity`, `ProductHealthResults`, `ProductTrayItems`, `ProductBestGameExe`, `ProductVersionText`, `ProductCenterGui`, `ProductTrayBaseTip`) exists purely so shared logic can ask a question each product answers differently. `LogLine` and `SharedPersistSettings` are the two oldest. Every name is verified present in **both** trees by `Assert-SharedParity`, so a shared body cannot quietly depend on something only one product defines. |
